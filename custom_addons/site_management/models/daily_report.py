...
class DailyReport(models.Model):
    ...
    name = fields.Char(
        ...
        required=True,
        default=lambda self: f"Reporte {fields.Date.today()}",
    )
    ...
    progress = fields.Float(
        string="Progreso (%)", required=True, help="Porcentaje de avance del proyecto"
    )
    ...
    team_size = fields.Integer(
        string="Personal en Obra", help="Número de trabajadores en sitio"
    )
    hours_worked = fields.Float(
        string="Horas Trabajadas", help="Total de horas hombre trabajadas"
    )
    ...
    def create(self, vals):
        if not vals.get("name"):
            vals["name"] = (
                f'Reporte {vals.get("date", fields.Date.today())} - {self.env.user.name}'
            )
    ...
    def _check_critical_incidents(self):
        ...
        if self.safety_incidents or self.issues:
            ...
            incident_data = {
                ...
                "severity": (
                    "high"
                    if "crítico" in (self.issues or self.safety_incidents).lower()
                    else "medium"
                ),
            }
            incident = self.env["ofitec.site.incident"].create_from_report(
                self.id, incident_data
            )

...
class WhatsAppConfig(models.Model):
    ...
    app_secret = fields.Char(string='App Secret', required=True,
                             help='WhatsApp Business App Secret')
    ...
    api_version = fields.Selection([
        ...
        ('v17.0', 'v17.0'),
        ('v16.0', 'v16.0'),
    ], string='API Version', default='v18.0', required=True)
    ...
    default_language = fields.Selection([
        ...
        ('en', 'English'),
        ('pt', 'Portuguese'),
    ], string='Default Language', default='es', required=True)

    # Statistics
    messages_sent_today = fields.Integer(string='Messages Sent Today',
                                         compute='_compute_stats', store=False)
    messages_received_today = fields.Integer(string='Messages Received Today',
                                             compute='_compute_stats', store=False)
    ...
    def send_template_message(self, to_phone, template_name, language_code=None,
                             components=None):
...
class WhatsAppMessage(models.Model):
    ...
    # Basic Information
    config_id = fields.Many2one('ofitec.whatsapp.config', string='WhatsApp Config',
    ...
    # Related Records
    next_action_id = fields.Many2one('ofitec.next.action', string='Related Action',
                                    ondelete='set null')
    ...
    def create_from_webhook(self, webhook_data, config):
        ...
        message_data = webhook_data.get('messages', [{}])[0] if webhook_data.get('messages') else {}
    ...
    def retry_send(self):
        ...
        for message in self.filtered(lambda m: m.direction == 'outbound' and m.status == 'failed'):

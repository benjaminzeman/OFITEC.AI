    ...
    try:
        ...
        with db.cursor() as cr:
            ...
            if groups:
                ...
                print("⚠️  No se encontraron grupos específicos, asignando grupos básicos")

                # Asignar grupos básicos
                basic_groups = env['res.groups'].search([('name', 'in', ['User types/Internal User', 'Technical/Show Accounting Features - Readonly'])])
                if basic_groups:
...
def create_custom_module_access(model_name, group_name="Internal User"):
    ...
    try:
        ...
        cur.execute(insert_query, (model_id, group_id, True, True, True, True, f"{model_name}.access_{model_name.replace('.', '_')}_internal_user"))

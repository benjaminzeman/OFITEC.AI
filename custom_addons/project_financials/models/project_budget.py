...
class ProjectBudget(models.Model):
    ...
    budget_amount = fields.Monetary(string='Monto Total del Presupuesto', required=True, currency_field='currency_id')
    currency_id = fields.Many2one('res.currency', string='Moneda', default=lambda self: self.env.company.currency_id)
    start_date = fields.Date(string='Fecha de Inicio', required=True)
    end_date = fields.Date(string='Fecha de Fin', required=True)

    # Tasas y costos
    cost_rate = fields.Monetary(string='Tasa de Costo por Hora', currency_field='currency_id')
    material_cost_rate = fields.Monetary(string='Tasa de Costos de Materiales', currency_field='currency_id')
    equipment_cost_rate = fields.Monetary(string='Tasa de Costos de Equipos', currency_field='currency_id')

    # Costos calculados automáticamente
    progress_cost = fields.Monetary(string='Costo Real del Progreso', compute='_compute_progress_cost', store=True, currency_field='currency_id')
    estimated_total_cost = fields.Monetary(string='Costo Total Estimado', compute='_compute_estimated_total_cost', store=True, currency_field='currency_id')
    variance_amount = fields.Monetary(string='Varianza (Presupuesto vs Real)', compute='_compute_variance', store=True, currency_field='currency_id')
    variance_percentage = fields.Float(string='Varianza (%)', compute='_compute_variance', store=True)
        ...
        for record in self:
            if record.project_id:
                ...
                for report in reports:
                    ...
                    material_cost = labor_cost * (record.material_cost_rate / 100) if record.material_cost_rate else 0
                    equipment_cost = labor_cost * (record.equipment_cost_rate / 100) if record.equipment_cost_rate else 0
    ...
    def _compute_estimated_total_cost(self):
        for record in self:
            if record.project_id and record.project_id.progress and record.project_id.progress > 0:
                # Estimar costo total basado en progreso actual
                progress_percentage = record.project_id.progress / 100
                if progress_percentage > 0:
                    record.estimated_total_cost = record.progress_cost / progress_percentage
                else:
    ...
    def _compute_variance(self):
        for record in self:
            if record.estimated_total_cost and record.budget_amount:
                record.variance_amount = record.estimated_total_cost - record.budget_amount
                record.variance_percentage = (record.variance_amount / record.budget_amount) * 100
            else:
    ...
    def update_costs_from_progress(self, project_id=None):
        ...
        if project_id:
            records = self.search([('project_id', '=', project_id), ('state', 'in', ['active', 'approved'])])
        else:

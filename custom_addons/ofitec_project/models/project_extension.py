...
class ProjectProject(models.Model):
    ...
    project_complexity = fields.Selection(
        ...
        string="Complejidad del Proyecto",
        default="medium",
    )
    ...
    budget_variance = fields.Float(
        string="Varianza Presupuestaria", compute="_compute_budget_variance"
    )
    ...
    def _compute_budget_variance(self):
        for record in self:
            if record.estimated_budget:
                record.budget_variance = (
                    (record.actual_budget - record.estimated_budget)
                    / record.estimated_budget
                ) * 100
            else:
                record.budget_variance = 0.0

    # Progreso de proyecto (para dashboards y reportes)
    progress = fields.Float(
        string="Progreso (%)",
        default=0.0,
        help="Avance global del proyecto en porcentaje",
    )

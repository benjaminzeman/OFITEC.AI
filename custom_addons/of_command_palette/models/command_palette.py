...
class CommandPalette(models.Model):
    ...
    context_fields = fields.Char(string='Campos de Contexto', help='Campos necesarios para ejecutar el comando')
    ...
    def _find_best_match(self, command_text):
        ...
        for command in self.search([('is_active', '=', True)]):
            score = self._calculate_similarity(command_text, command.name + ' ' + (command.keywords or ''))
    ...
    def _ai_powered_processing(self, command_text, context=None):
        ...
        if intent == 'project_status':
            ...
            return ai_bridge.predict_costs(context.get('project_id') if context else None)
        else:
            return self._basic_processing(command_text, context)
    ...
    def _analyze_intent(self, command_text):
        ...
        elif any(word in text for word in ['costo', 'cost', 'presupuesto', 'budget', 'gasto']):
    ...
    def _get_project_status(self, context=None):
        ...
        active_projects = len(projects.filtered(lambda p: p.state not in ['done', 'cancelled']))
    ...
    def _get_cost_analysis(self, context=None):
        ...
        return {
            'type': 'cost_analysis',
            'message': 'No hay presupuestos activos'
        }
    ...
    def _create_daily_report(self, context=None):
        ...
        report_data = {
            ...
            'description': context.get('description', 'Reporte generado por Command Palette'),
            'state': 'draft'
        }

...
class RiskRecord(models.Model):
    ...
    probability_value = fields.Float(
        string="Probabilidad Numérica (%)",
        compute="_compute_probability_value",
        store=True,
    )
    impact_value = fields.Monetary(
        string="Impacto Monetario", required=True, currency_field="currency_id"
    )
    ...
    risk_exposure = fields.Monetary(
        string="Exposición al Riesgo",
        compute="_compute_risk_exposure",
        store=True,
    ...
    identification_date = fields.Date(
        string="Fecha de Identificación", default=fields.Date.today, required=True
    )
    ...
    def _compute_risk_exposure(self):
        for record in self:
            record.risk_exposure = (
                record.probability_value / 100
            ) * record.impact_value

    @api.model
    def create(self, vals):
        if not vals.get("name"):
            category = dict(self._fields["risk_category"].selection).get(
                vals.get("risk_category"), "Riesgo"
            )
            vals["name"] = (
                f'{category} - {vals.get("identification_date", fields.Date.today())}'
            )
    ...
    def action_start_mitigation(self):
        ...
        self.write(
            {"status": "mitigating", "mitigation_start_date": fields.Date.today()}
        )

    def action_complete_mitigation(self):
        """Completar mitigación del riesgo"""
        self.write({"status": "mitigated", "mitigation_end_date": fields.Date.today()})
    ...
    def action_risk_occurred(self):
        ...
        self.write(
            {"status": "occurred", "actual_occurrence_date": fields.Date.today()}
        )
        ...
        return self.create(
            {
                ...
                "risk_category": self._map_incident_to_risk_category(
                ...
                "responsible_id": (
                    incident.responsible_id.id
                    if incident.responsible_id
                    else incident.user_id.id
                ),
    ...
    def generate_risk_report(self, project_id):
        ...
        return {
            ...
            "high_severity": len(
                risks.filtered(lambda r: r.severity in ["high", "critical"])
            ),

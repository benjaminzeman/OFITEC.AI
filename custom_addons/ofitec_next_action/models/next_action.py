...
class NextAction(models.Model):
    ...
    priority = fields.Selection([
        ...
        ('3', 'Media (3)'),
        ('4', 'Baja (4)')
    ], string='Prioridad', required=True, default='3')
    ...
    budget_id = fields.Many2one('ofitec.project.financials', string='Presupuesto Relacionado')
    ...
    # Fechas importantes
    suggested_date = fields.Date(string='Fecha Sugerida', default=fields.Date.today, required=True)
    ...
    def _analyze_critical_risks(self, project_id=None):
        ...
        domain = [('severity', 'in', ['high', 'critical']), ('status', 'not in', ['closed', 'occurred'])]
        if project_id:
        ...
        for risk in critical_risks:
            ...
            actions.append({
                ...
                'recommended_actions': risk.mitigation_plan or 'Revisar y ejecutar plan de mitigación definido',
                ...
                'deadline': risk.deadline or (datetime.now().date() + timedelta(days=7)),
        ...
        for budget in budgets:
            if budget.variance_percentage and budget.variance_percentage > 15:
                actions.append({
                    ...
                    'action_type': 'urgent' if budget.variance_percentage > 25 else 'planned',
                    ...
                    'impact_score': 8.0,
                    'urgency_score': 9.0 if budget.variance_percentage > 25 else 6.0,
    ...
    def _analyze_active_incidents(self, project_id=None):
        ...
        for incident in active_incidents:
            ...
            action_type = 'urgent' if incident.severity in ['high', 'critical'] else 'planned'
            ...
            actions.append({
                ...
                'urgency_score': 8.0 if incident.severity in ['high', 'critical'] else 5.0,
                'recommended_actions': incident.preventive_action or 'Investigar causa raíz y implementar solución',
                ...
                'deadline': incident.deadline or (datetime.now().date() + timedelta(days=3)),
    ...
    def _analyze_project_progress(self, project_id=None):
        ...
        projects = self.env['project.project'].search(domain) if project_id else self.env['project.project'].search([])
    ...
    def get_dashboard_data(self):
        ...
        critical_count = self.search_count([('priority', '=', '1'), ('status', '=', 'pending')])
        high_count = self.search_count([('priority', '=', '2'), ('status', '=', 'pending')])

        # Próximas acciones urgentes
        urgent_actions = self.search([
            ('status', '=', 'pending'),
            ('priority', 'in', ['1', '2'])
        ], order='priority, suggested_date', limit=5)
        ...
        return {
            ...
            'urgent_actions': [{
                ...
                'project': action.project_id.name if action.project_id else 'General'

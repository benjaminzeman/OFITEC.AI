...
class ExecutiveDashboardController(http.Controller):
    ...
    def _get_kpi_data(self, project_id):
        ...
        if project_id:
            ...
            progress = sum(p.progress for p in projects) / len(projects) if projects else 0
        ...
        return {
            ...
            'avg_variance': 12.5  # Placeholder - calcular desde presupuestos
        }
    ...
    def get_risks_chart_data(self):
        ...
        categories = {}
        for risk in risks:
            cat = dict(risk._fields['risk_category'].selection).get(risk.risk_category, 'Otro')
            categories[cat] = categories.get(cat, 0) + 1

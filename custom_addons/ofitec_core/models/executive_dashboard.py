...
class ExecutiveDashboard(models.Model):
    ...
    total_projects = fields.Integer(string='Total de Proyectos', compute='_compute_totals')
    active_projects = fields.Integer(string='Proyectos Activos', compute='_compute_totals')
    completed_reports = fields.Integer(string='Reportes Completados (Mes)', compute='_compute_totals')
    total_risks = fields.Integer(string='Total Riesgos Activos', compute='_compute_totals')
    critical_risks = fields.Integer(string='Riesgos Críticos', compute='_compute_totals')
    total_budget = fields.Monetary(string='Presupuesto Total', compute='_compute_financials')
    total_cost = fields.Monetary(string='Costo Total Real', compute='_compute_financials')
    avg_variance = fields.Float(string='Varianza Promedio (%)', compute='_compute_financials')
    ...
    def get_dashboard_summary(self):
        ...
        return {
            ...
            'reports': {
                'completed_this_month': self.completed_reports
            },
            'risks': {
    ...
    def get_recent_activity(self):
        ...
        recent_incidents = self.env['ofitec.site.incident'].search(
            [('state', '=', 'resolved'), ('date', '>=', datetime.now().date() - timedelta(days=7))],
            order='date desc',
            limit=2
        )

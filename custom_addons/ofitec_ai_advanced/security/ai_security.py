...
def check_ai_access(env, user_id):
    ...
    manager_group = env.ref(
        "ofitec_ai_advanced.group_ai_manager", raise_if_not_found=False
    )
    ...
    def check(self, model_name, mode="read", raise_exception=True):
        ...
        if model_name.startswith("ofitec.ai"):
            ...
            if not check_ai_access(self.env, user_id):
                if raise_exception:
                    raise AccessError(
                        "Access denied. AI features require specific permissions."
                    )
                return False
...
class ResUsers(models.Model):
    ...
    def _generate_api_token(self):
        ...
        self.write(
            {"api_token": token_hash, "api_token_expires": time.time() + 3600}  # 1 hour
        )
...
class AIWebhookSecurity(models.Model):
    ...
    ip_whitelist = fields.Text(
        string="IP Whitelist", help="Comma-separated list of allowed IP addresses"
    )
    rate_limit = fields.Integer(
        string="Rate Limit", default=100, help="Maximum requests per hour"
    )
    ...
    def _generate_signature(self, data):
        ...
        signature = hmac.new(
            self.secret_token.encode(), payload.encode(), hashlib.sha256
        ).hexdigest()
...
class AISecurityAudit(models.Model):
    ...
    operation_time = fields.Datetime(
        string="Operation Time", default=fields.Datetime.now
    )
    ip_address = fields.Char(string="IP Address")
    user_agent = fields.Char(string="User Agent")
